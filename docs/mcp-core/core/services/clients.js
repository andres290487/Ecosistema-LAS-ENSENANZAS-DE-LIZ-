...
const tronClients = new Map();
const evmPublicClients = new Map();
function getTronClient(network) {
    if (tronClients.has(network))
        return tronClients.get(network);
...
export function getPublicClient(network) {
    const config = getNetworkConfig(network);
    if (config.kind === "tron")
        return getTronClient(network);
    if (evmPublicClients.has(network))
        return evmPublicClients.get(network);
