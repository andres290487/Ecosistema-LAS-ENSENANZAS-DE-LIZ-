...
async function waitForTronReceipt(network, txID) {
    const tronWeb = getPublicClient(network);
...
export async function readContract(params) {
    ...
    if (config.kind === "tron") {
        const tronWeb = getPublicClient(params.network);
        const contract = tronWeb.contract(params.abi, params.address);
...
export async function writeContract(params) {
    ...
    const args = params.args || [];
    if (config.kind === "tron") {
        const tronWeb = getWalletClient(params.network);
        const contract = tronWeb.contract(params.abi, params.address);
    ...
    const walletClient = getWalletClient(params.network);
    ...
    const publicClient = getPublicClient(params.network);
    const receipt = await publicClient.waitForTransactionReceipt({ hash });
    return { txID: hash, receipt };
}
export async function getProxyAddress(network, buildIfMissing = false) {
    ...
    if (normalized && normalized !== empty && !/^0x0+$/.test(normalized))
        return normalized;
    if (!buildIfMissing)
        return null;
    await writeContract({
...
export async function ensureProxy(network) {
...
export async function executeProxyAction(params) {
    ...
    const execArgs = [utils.toEncodedAddress(params.target, params.network), data];
    const config = getNetworkConfig(params.network);
    if (config.kind === "tron") {
