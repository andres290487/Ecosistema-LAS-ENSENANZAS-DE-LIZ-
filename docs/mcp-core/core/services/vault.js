...
import { getIlkConfig, getNetworkConfig } from "../chains.js";
...
function summarizeRisk(healthFactor) {
...
async function assertVaultOwnedByCurrentProxy(network, cdpId) {
...
async function getVaultPositionRaw(network, cdpId) {
    const config = getNetworkConfig(network);
...
async function ensureUsddApprovalToProxy(network, requiredRaw, proxy) {
    const config = getNetworkConfig(network);
...
export async function getUserVaultIds(network, address) {
    const config = getNetworkConfig(network);
    ...
    if (count === 0n || first === 0n)
        return [];
    const ids = [];
    let current = first;
    while (current !== 0n && ids.length < Number(count)) {
        ids.push(current);
        const list = await readContract({ network, address: config.cdpManager, abi: CDP_MANAGER_ABI, functionName: "list", args: [current] });
        current = BigInt((list[1] ?? list.next ?? 0).toString());
    }
    return ids;
}
export async function getVaultSummary(network, cdpId) {
    const config = getNetworkConfig(network);
...
export async function analyzeVaultRisk(network, cdpId) {
    const summary = await getVaultSummary(network, cdpId);
    const warnings = [];
    if (summary.riskLevel === "critical")
        warnings.push("Vault is within liquidation danger range.");
    if (summary.riskLevel === "high")
        warnings.push("Vault is close to liquidation threshold; reduce debt or add collateral.");
    if (summary.riskLevel === "medium")
        warnings.push("Vault is serviceable but should be monitored for oracle moves.");
    if (summary.riskLevel === "healthy")
        warnings.push("Vault currently has a reasonable collateral buffer.");
    return { ...summary, warnings };
}
async function findExistingVaultForIlk(network, ilkKey) {
    const config = getNetworkConfig(network);
...
export async function openVault(network, ilk) {
    const config = getNetworkConfig(network);
...
export async function drawUsdd(network, cdpId, amount) {
    const config = getNetworkConfig(network);
...
export async function repayUsdd(network, cdpId, amount) {
    const config = getNetworkConfig(network);
...
export async function withdrawCollateral(network, cdpId, ilk, amount) {
    const config = getNetworkConfig(network);
    await assertVaultOwnedByCurrentProxy(network, cdpId);
    const ilkConfig = getIlkConfig(ilk, network);
...
export async function closeVault(network, cdpId, ilk, amountToFree) {
    const config = getNetworkConfig(network);
    const proxy = await assertVaultOwnedByCurrentProxy(network, cdpId);
    const ilkConfig = getIlkConfig(ilk, network);
