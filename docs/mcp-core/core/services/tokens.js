...
async function assertProtocolSpender(network, spender) {
    const config = getNetworkConfig(network);
    const allowed = new Set();
    const normalize = (value) => utils.normalizeAddress(value, network);
    allowed.add(normalize(config.usddJoin));
    allowed.add(normalize(config.proxyRegistry));
    allowed.add(normalize(config.proxyActions));
    if (config.proxyActionsProxy)
        allowed.add(normalize(config.proxyActionsProxy));
