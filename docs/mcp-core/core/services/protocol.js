...
import { getIlkConfig, getNetworkConfig, getSupportedIlks, getSupportedPsmMarkets } from "../chains.js";
import { readContract } from "./contracts.js";
import { getConfiguredWallet } from "./wallet.js";
import { utils } from "./utils.js";
async function readOraclePriceSnapshot(network, pip) {
...
export async function getProtocolOverview(network) {
    ...
    const [vatLine, protocolDebt, dogHole, ...ilkResults] = await Promise.all([
        ...
        ...ilkList.map((ilk) => readContract({ network, address: config.vat, abi: VAT_ABI, functionName: "ilks", args: [utils.toBytes32(ilk.key)] })),
    ]);
    const ilksWithDust = ilkList.map((ilk, i) => {
        const ilkData = ilkResults[i];
...
export async function getOracleStatus(network, ilk) {
    const config = getNetworkConfig(network);
...
export async function getPsmStatus(network, market) {
    ...
    if (!psm)
        throw new Error(`Unknown PSM market: ${market}`);
...
export async function getSavingsStatus(network) {
    const config = getNetworkConfig(network);
    if (!config.savings) {
