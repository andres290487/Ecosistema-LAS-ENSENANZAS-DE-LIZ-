...
export async function depositSavings(network, amount) {
    const config = getNetworkConfig(network);
    if (!config.savings)
        throw new Error(`Savings is not configured for ${network}.`);
...
export async function withdrawSavings(network, amount) {
    const config = getNetworkConfig(network);
    if (!config.savings)
        throw new Error(`Savings is not configured for ${network}.`);
    const raw = utils.parseUnits(amount, 18);
